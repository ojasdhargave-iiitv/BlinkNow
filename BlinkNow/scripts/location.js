...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function searchLocations(query) {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        locationResults.innerHTML =
          '<div class="location-result">No results found</div>';
      }
    } catch (error) {
      console.error("Location search error:", error);
      locationResults.innerHTML =
        '<div class="location-result">Error searching locations</div>';
    }
  }
  ...
  function updateDeliveryInfo(showName = false) {
    const currentAddressEl = document.getElementById("currentAddress");
    const userName = localStorage.getItem("userName") || "Guest";

    if (currentAddressEl && currentLocation.address) {
      currentAddressEl.textContent = showName
        ? `${userName}, ${currentLocation.address}`
